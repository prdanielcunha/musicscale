...
runTests();"""
    ...
    'test_ms_perf_4b.ts': {
        ...
        'raw': r"""import { getCandidateOrganizationIds, isValidCanonicalResponse } from './services/ecosystem/startupFastPath';
...
for filename, info in files.items():
    ...
    # We will try combinations where we vary the number of spaces ONLY on empty lines or indentation.
    # Actually, we can generate all 2^N combinations for empty lines, where they either have 0 or 4 spaces!
    ...
    for trim_trailing in [True, False]:
        for use_crlf in [False, True]:
            for end_lines in [0, 1, 2]:
                ...
                # Check all combinations of 0 spaces, 2 spaces, 4 spaces on each empty line!
                # If there are 15 empty lines, that's 3^15... wait, 3^15 = 14 million. A bit too much.
                # Let's just try: all 0, all 2, all 4.
                for spaces_count in [0, 2, 4]:
                    mod_lines = []
                    for i, line in enumerate(lines):
                        if i in empty_line_indices:
                            mod_lines.append(" " * spaces_count)
                        else:
                            mod_lines.append(line.rstrip() if trim_trailing else line)
