...
const ProfilePage: React.FC = () => {
  ...
  const generateAndGetInviteLink = async () => {
     if (!userProfile || !organization) return null;
     setIsGeneratingInvite(true);
     try {
         const { generateInvite } = await import('../services/inviteService');
         // default role generic 'member' or 'visitor', relying on seed
         const invite = await generateInvite(userProfile, organization.id, orgName, 'member', 'Membro', 7);
         if (!invite) throw new Error('Failed to generate invite');
         const baseUrl = window.location.origin;
         return `${baseUrl}/join?invite=${invite.token!}`;
     } catch(e) {
         logger.error(e);
  ...
  const handleWhatsAppInvite = async () => {
    const link = await generateAndGetInviteLink();
    if (!link) return;
    
    const text = encodeURIComponent(
      `Olá! Você foi convidado(a) para fazer parte da organização ${orgName} no MusicScale.\n\nO MusicScale ajuda nossa equipe de louvor a organizar escalas, repertório, cifras e ministrações com mais excelência.\n\nEntre pelo link abaixo para aceitar o convite e já ficar vinculado(a) automaticamente à nossa organização:\n\n${link}\n\nSe ainda não tiver conta, é só se cadastrar usando esse link.`
    );
  ...
  const handleEmailInvite = async () => {
    ...
    const subject = encodeURIComponent(
      `Convite para se juntar à ${orgName} no MusicScale`
    );
    const body = encodeURIComponent(
      `Olá!\n\nVocê foi convidado(a) para fazer parte da organização ${orgName} no MusicScale.\n\nO MusicScale ajuda nossa equipe de louvor a organizar escalas, repertório, cifras e ministrações com mais excelência.\n\nEntre pelo link abaixo para aceitar o convite e já ficar vinculado(a) automaticamente à nossa organização:\n\n${link}\n\nSe ainda não tiver conta, é só se cadastrar usando esse link.\n\nAté logo!`
    );
              ...
              <form onSubmit={handleOrgSubmit}>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="pt-2">
                    ...
                    <p className="text-sm text-slate-500 dark:text-gray-400 mb-4">
                      {t("profile.invite_members_desc", "Compartilhe o link de convite abaixo. Seus convidados entrarão automaticamente na organização.")}
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      >
                        {isGeneratingInvite ? <Spinner size="sm" /> : <LinkIcon className="w-4 h-4" />}
                        {showInviteCopied ? t("common.copied", "Copiado!") : t("common.link", "Link")}
                      </Button>

                      <Button
                        type="button"
                        variant="secondary"
                        className="w-full justify-center gap-2 py-3 bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 border-[#25D366]/20 px-1 disabled:opacity-50"
                        onClick={handleWhatsAppInvite}
                        disabled={isGeneratingInvite}
                      >
                        {isGeneratingInvite ? <Spinner size="sm" /> : (
                          <>
                      ...
                      >
                        {isGeneratingInvite ? <Spinner size="sm" /> : <Mail className="w-4 h-4" />}
                        Email
                      </Button>
                    </div>
                  </div>
