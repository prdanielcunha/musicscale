...
export function MedleyStage({ medley, scaleId, publishRevision }: { medley: ScaleMedley; scaleId?: string; publishRevision?: number; key?: string }) {
  ...
  const exportPresentation = () => {
    const url = URL.createObjectURL(new Blob([medleyPresentationHtml(medley)], { type: 'text/html;charset=utf-8' }));
  ...
  const navigate = async (direction: -1 | 1) => {
    ...
    if (live.isLive && live.canControlLiveSession && !stageRevisionChanged && publishRevision) {
      const sent = await live.directMedleyStep(medley.id, medley.steps[target].id, nextRound, publishRevision);
    ...
    if (target !== position) setClickStepId(null);
    setPosition(target);
    setRound(nextRound);
  };
  useEffect(() => {
    ...
    if (resolved.index !== position) setClickStepId(null);
  ...
  return <>
    <button type="button" className="w-full rounded-2xl border border-indigo-400/30 bg-indigo-500/10 p-4 text-left text-white" onClick={() => { setPosition(0); setRound(1); setClickStepId(null); lastSequence.current = 0; setOpen(true); }}>
    ...
    {open && step && <div role="dialog" aria-modal="true" aria-label={t('medley.title')} className="fixed inset-0 z-[120] flex flex-col bg-[#090b12] text-white">
      ...
      {live.isLive && <div className="flex flex-wrap items-center gap-3 border-b border-white/10 px-4 py-2 text-xs">
        ...
        {live.canControlLiveSession && publishRevision && !stageRevisionChanged && <button type="button" className="rounded-lg border border-indigo-400/30 px-3 py-2" onClick={async () => { const sent = await live.directMedleyStep(medley.id, step.id, round, publishRevision); setDirectionError(!sent); if (sent) setIsFollowingDirection(true); }}>{t('medley.broadcastCurrent')}</button>}
      </div>}
      ...
      {step.bpm && <div className="border-b border-white/10 px-4 py-2 text-xs"><button type="button" className="rounded-lg border border-white/20 px-3 py-2" onClick={() => setClickStepId(current => current === step.id ? null : step.id)}>{clickStepId === step.id ? t('medley.hideClick') : t('medley.showClick')}</button><span className="ml-3 text-white/60">{t('medley.clickHelp')}</span>{clickStepId === step.id && <div className="mt-2 max-w-md"><Metronome key={step.id} initialBpm={step.bpm} /></div>}</div>}
