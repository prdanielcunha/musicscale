...
export const CandidateDetailsModal: React.FC<CandidateDetailsModalProps> = ({ candidateId, onClose, onApproveSuccess, onLinkSuccess, onRejectSuccess }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (isMounted) setError("Falha ao carregar os detalhes. Tente novamente.");
            } finally {
    ...
    const translateStatus = (status: string) => {
        return translateClassification(status);
    };
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            setApproveError(e.message || "Erro desconhecido ao aprovar.");
        } finally {
        ...
        try {
            ...
            setShowLinkConfirm(false);
             ...
             if (e.requiresConfirmation) {
                 ...
                 setLinkError(e.message || "A correspondência é apenas moderada. Confirme abaixo se deseja prosseguir.");
             } else {
                 setLinkError(e.message || "Erro desconhecido ao vincular.");
             }
        } finally {
            setIsLinking(false);
        }
    ...
    const handleReject = async () => {
        if (!rejectReasonCode) {
            setRejectError("Selecione um motivo para rejeição.");
            return;
        }
        setIsRejecting(true);
        setRejectError(null);
        ...
        try {
            ...
            setShowRejectConfirm(false);
             ...
             setRejectError(e.message || "Erro desconhecido ao rejeitar candidata.");
        } finally {
            ...
            >
                <div className="sticky top-0 z-10 bg-white dark:bg-[#0A0A0C] flex flex-col px-6 py-4 border-b border-slate-100 dark:border-white/5">
                    <div className="flex justify-between items-center mb-3">
                        <h2 className="text-lg font-bold">Detalhes da Candidata</h2>
                ...
                {approveError && (
                    <div className="mx-6 mt-4 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-between">
                        ...
                        <button onClick={() => setApproveError(null)} className="text-red-500 hover:text-red-600 font-bold text-xs uppercase tracking-wider">Dispensar</button>
                    </div>
                )}
                ...
                {linkError && (
                    <div className="mx-6 mt-4 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center justify-between">
                        ...
                        <button onClick={() => setLinkError(null)} className="text-amber-600 dark:text-amber-500 hover:text-amber-700 font-bold text-xs uppercase tracking-wider">Dispensar</button>
                    </div>
                )}
                ...
                {showConfirm && (
                    <div className="absolute inset-0 z-50 bg-white/90 dark:bg-[#0A0A0C]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center animate-fade-in">
                         <div className="max-w-md w-full bg-white dark:bg-[#1A1A1A] p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-white/10">
                              <h3 className="text-xl font-bold mb-2">Aprovar Candidata</h3>
                              <p className="text-slate-500 text-sm mb-6">
                                Você está prestes a adicionar <strong>"{details?.title}"</strong> ({details?.artist}) ao Acervo Oficial da Biblioteca Viva.
                              </p>
                              
                              <div className="bg-slate-50 dark:bg-black/30 p-4 rounded-xl mb-6 text-left border border-slate-100 dark:border-white/5">
                                 <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-2">Ocorrência Base Selecionada</p>
                                 <p className="text-sm font-medium">{activeOccurrenceId || 'Primeira Disponível'}</p>
                              </div>
                              ...
                              <div className="flex gap-3 w-full">
                                  ...
                                  >
                                      {isApproving ? <Spinner size="sm" /> : 'Confirmar'}
                                  </button>
                              </div>
                         </div>
                    </div>
                ...
                {showLinkConfirm && (
                    <div className="absolute inset-0 z-50 bg-white/90 dark:bg-[#0A0A0C]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center animate-fade-in">
                         <div className="max-w-md w-full bg-white dark:bg-[#1A1A1A] p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-white/10">
                              <h3 className="text-xl font-bold mb-2">Vincular Candidata</h3>
                              <p className="text-slate-500 text-sm mb-6">
                                Você está prestes a fundir <strong>"{details?.title}"</strong> com uma música já existente na Biblioteca Viva.
                              </p>
                              
                              {forceModeratedMatch && (
                                <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl mb-6 text-left">
                                    <p className="text-sm text-amber-600 font-bold">Atenção: Correspondência Moderada</p>
                                    <p className="text-xs text-amber-700/80 mt-1">Ao confirmar, você atesta que revisou manualmente o conteúdo e confirmou se tratar da mesma obra.</p>
                                </div>
                              )}
                              ...
                              <div className="flex gap-3 w-full">
                                  ...
                                  >
                                      {isLinking ? <Spinner size="sm" /> : (forceModeratedMatch ? 'Forçar Vínculo' : 'Confirmar')}
                                  </button>
                              </div>
                         </div>
                    </div>
                ...
                {showRejectConfirm && (
                    <div className="absolute inset-0 z-50 bg-white/90 dark:bg-[#0A0A0C]/90 backdrop-blur-md flex flex-col justify-center items-center p-6 text-center animate-fade-in overflow-y-auto">
                         <div className="max-w-md w-full bg-white dark:bg-[#1A1A1A] p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-white/10 text-left my-auto">
                              <h3 className="text-xl font-bold mb-2">Rejeitar Candidata</h3>
                              <p className="text-slate-500 text-sm mb-6 font-medium">
                                Você está prestes a descartar <strong>"{details?.title}"</strong>. A candidata sairá da fila ativa, mas manterá o histórico.
                              </p>
                              
                              <div className="mb-4">
                                  <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Motivo (Obrigatório)</label>
                                  <select 
                                  ...
                                  >
                                      ...
                                      <option value="duplicate_candidate">Candidata Duplicada</option>
                                      <option value="invalid_content">Conteúdo Inválido/Estragado</option>
                                      <option value="insufficient_content">Conteúdo Insuficiente</option>
                                      <option value="medley_or_compilation">Medley ou Compilação (Não Suportado)</option>
                                      <option value="not_a_song">Não é uma Música</option>
                              ...
                              <div className="mb-6">
                                  <div className="flex justify-between items-center mb-2">
                                      <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">Nota Privada (Opcional)</label>
                                  ...
                                  <textarea 
                                      ...
                                      maxLength={500}
                                      placeholder="Detalhes adicionais para histórico interno..."
                              ...
                              {rejectError && (
                                <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex flex-col gap-2">
                                    <p className="text-red-600 dark:text-red-500 text-sm font-bold">Erro ao rejeitar:</p>
                              ...
                              <div className="flex gap-3 w-full">
                                  ...
                                  >
                                      {isRejecting ? <Spinner size="sm" /> : 'Rejeitar'}
                                  </button>
                              </div>
                         </div>
                    </div>
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] shrink-0">
                            <div className="flex items-center justify-between mb-4">
                                <div>
                                    <h3 className="text-2xl font-bold flex items-center gap-3">
                                        ...
                                        {details.errorMsg && <span className="bg-red-500/10 text-red-500 px-2 py-0.5 rounded text-xs uppercase" title={details.errorMsg}>Erro</span>}
                                    </h3>
                            ...
                            <div className="flex space-x-1 overflow-x-auto hide-scrollbar">
                                {[
                                    ...
                                    { id: 'matches', label: 'Correspondências' },
                                    { id: 'occurrences', label: `Ocorrências (${occurrences.length})` },
                        ...
                        <div className="flex-1 overflow-y-auto bg-white dark:bg-[#0A0A0C] shrink">
                            {activeTab === 'overview' && (
                                <div className="p-6 space-y-6 animate-fade-in">
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="bg-slate-50 dark:bg-white/[0.03] rounded-2xl p-4 border border-slate-100 dark:border-white/5">
                                            <p className="text-xs uppercase font-bold text-slate-400 mb-1">Descoberta em</p>
                                            <p className="font-semibold text-sm truncate">{details.firstDiscoveredAt ? new Date(details.firstDiscoveredAt).toLocaleDateString() : 'Não informado'}</p>
                                        </div>
                                        <div className="bg-slate-50 dark:bg-white/[0.03] rounded-2xl p-4 border border-slate-100 dark:border-white/5">
                                            <p className="text-xs uppercase font-bold text-slate-400 mb-1">Ocorrências</p>
                                            <p className="font-semibold text-sm truncate">{details.occurrenceCount || occurrences.length}</p>
                                        </div>
                                        <div className="bg-slate-50 dark:bg-white/[0.03] rounded-2xl p-4 border border-slate-100 dark:border-white/5">
                                            <p className="text-xs uppercase font-bold text-slate-400 mb-1">Organizações</p>
                                    ...
                                    <div className="bg-slate-50 dark:bg-white/[0.03] rounded-3xl p-6 border border-slate-100 dark:border-white/5">
                                        <h3 className="font-bold text-lg mb-4">Informações Musicais</h3>
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-2">
                                            <div>
                                                <p className="text-xs text-slate-500 font-medium">BPM</p>
                                                <p className="font-bold">{details.snapshot?.bpm || details.analysisSummary?.metrics?.avgBpm ? Math.round(details.snapshot?.bpm || details.analysisSummary?.metrics?.avgBpm) : 'Não informado'}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 font-medium">Ritmo</p>
                                                <p className="font-bold">{details.snapshot?.timeSignature || 'Não informado'}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 font-medium">Tom Atual</p>
                                                <p className="font-bold">{details.snapshot?.key || details.analysisSummary?.metrics?.mostCommonKey || 'Não informado'}</p>
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-500 font-medium">Tom Original</p>
                                                <p className="font-bold">{details.snapshot?.originalKey || 'Não informado'}</p>
                                            </div>
                                            <div className="col-span-2">
                                                <p className="text-xs text-slate-500 font-medium">Idioma</p>
                                                <p className="font-bold">{details.snapshot?.language || 'Não informado'}</p>
                                            </div>
                                            <div className="col-span-2">
                                                <p className="text-xs text-slate-500 font-medium">Tags</p>
                                                <p className="font-bold">{details.snapshot?.tags?.join(', ') || 'Não informado'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {details.analysisSummary?.matchDetails?.reason && (
                                        <div className="bg-blue-500/5 border border-blue-500/10 rounded-2xl p-5">
                                           <h4 className="text-blue-500 dark:text-blue-400 font-bold text-sm mb-2">Motivo do Match Principal</h4>
                                    ...
                                    {details.errorMsg && (
                                        <div className="bg-red-500/5 border border-red-500/10 rounded-2xl p-5">
                                           <h4 className="text-red-500 font-bold text-sm mb-2">Erro de Processamento</h4>
                                ...
                                <div className="p-6 animate-fade-in font-sans">
                                    {parsedChords.length > 0 ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                                            ...
                                            <p className="font-medium">Nenhuma cifra disponível na visualização da candidata.</p>
                                        </div>
                                    )}
                                </div>
                            )}
                                ...
                                <div className="p-6 animate-fade-in text-left">
                                    {details.snapshot?.lyrics || activeOccurrence?.lyrics ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                                            ...
                                            <p className="font-medium">Nenhuma letra disponível nesta ocorrência.</p>
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            {activeTab === 'matches' && (
                                <div className="p-6 space-y-4 animate-fade-in">
                                    {matches.length === 0 ? (
                                        <div className="text-center py-12 text-slate-400 font-medium">Nenhuma correspondência relevante analisada.</div>
                                    ) : (
                                        matches.map(m => (
                                                   ...
                                                   <div className="flex-1">
                                                       {m.__globalSongDetails ? (
                                                            <>
                                                                ...
                                                                <p className="text-sm text-slate-500 mt-1">
                                                                    {m.__globalSongDetails.artist || 'Desconhecido'}
                                                                </p>
                                                            </>
                                                       ) : (
                                                       ...
                                                       <div className="flex flex-col mt-4">
                                                            <span className="text-xs text-slate-500 tracking-wide">
                                                                CONFIANÇA: <span className="font-bold text-slate-700 dark:text-slate-300 ml-1">{Math.round((m.score || 0)*100)}%</span>
                                                            </span>
                                                            <p className="font-medium text-xs text-slate-600 dark:text-slate-400 mt-1.5">{translateReason(m.reason || 'Desconhecido')}</p>
                                                       </div>
                                                   </div>
                                                   <span className="text-xs font-mono text-slate-400 bg-slate-100 dark:bg-black/40 px-2 py-1 rounded-md shrink-0">
                                                       {m.globalSongId || 'ID oculto'}
                                                   </span>
                                               </div>
                            ...
                            {activeTab === 'occurrences' && (
                                <div className="p-6 space-y-4 animate-fade-in">
                                    {occurrences.length === 0 ? (
                                        <div className="text-center py-12 text-slate-400 font-medium">Nenhuma ocorrência encontrada.</div>
                                    ) : (
                                        occurrences.map(o => (
                                            <div key={o.id} className="p-5 rounded-2xl bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 space-y-3 shadow-sm">
                                               <div className="flex justify-between items-start mb-2">
                                                    <div>
                                                        <p className="font-bold text-base">{o.__organizationName || 'Desconhecido'}</p>
                                                        <p className="text-xs text-slate-500 mt-0.5">{o.snapshot?.title} — {o.snapshot?.artist}</p>
                                                    </div>
                                                    <span className="text-xs font-medium text-slate-400 shrink-0">
                                                        {o.createdAt?.seconds ? new Date(o.createdAt.seconds * 1000).toLocaleDateString() : 'Não informada'}
                                                    </span>
                                               </div>
                                               ...
                                               <div className="flex flex-wrap gap-2 mb-3">
                                                   <span className="text-xs font-bold text-slate-500 bg-slate-100 dark:bg-white/5 dark:text-slate-400 px-2.5 py-1 rounded-md uppercase tracking-wide">
                                                       {o.discovery?.origin === 'app_creation' ? 'Criação no App' : 
                                                        o.discovery?.origin === 'system_scan' ? 'Varredura Sistêmica' : 
                                                        o.discovery?.origin === 'migration' ? 'Migração' : 'Origem Desconhecida'}
                                                   </span>
                                               </div>

                                               <div className="flex gap-4 border-t border-slate-100 dark:border-white/5 pt-3">
                                                   <span className="text-sm flex items-center gap-1.5"><span className={`w-2 h-2 rounded-full ${o.snapshot?.chords ? 'bg-green-500' : 'bg-slate-300 dark:bg-slate-600'}`}></span> {o.snapshot?.chords ? 'Com Cifra' : 'Sem Cifra'}</span>
                                                   <span className="text-sm flex items-center gap-1.5"><span className={`w-2 h-2 rounded-full ${o.snapshot?.lyrics ? 'bg-green-500' : 'bg-slate-300 dark:bg-slate-600'}`}></span> {o.snapshot?.lyrics ? 'Com Letra' : 'Sem Letra'}</span>
                                               </div>
                                            </div>
                                        ))
                                    )}
                            ...
                            {activeTab === 'history' && (
                                <div className="p-6 space-y-4 animate-fade-in relative z-0">
                                    {history.length === 0 ? (
                                        <div className="text-center py-12 text-slate-400 font-medium">Nenhum evento registrado.</div>
                                    ) : (
                                        history.map(h => {
                                            const eventName = h.eventType === 'occurrence_added' ? 'Ocorrência Adicionada' :
                                                              h.eventType === 'created' ? 'Candidata Criada' :
                                                              h.action === 'approved' ? 'Aprovada' :
                                                              h.action === 'rejected' ? 'Rejeitada' :
                                                              h.action === 'linked' ? 'Vinculada' :
                                                              h.eventType || h.action || 'Evento do Sistema';
                                            
                                            const timeDisplay = h.timestampMillis ? new Date(h.timestampMillis).toLocaleString() : 'Data indisponível';
                                            ...
                                            return (
                                                <div key={h.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5">
                                                   ...
                                                   {h.reasonCode && (
                                                       <p className="text-xs text-slate-500 font-medium mb-1">Motivo: {h.reasonCode}</p>
                                                   )}
                                                   {h.privateNote && (
                                                       <div className="mt-2 p-3 bg-slate-100 dark:bg-black/20 rounded-xl text-xs text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/5">
                                                            <span className="font-bold block mb-1">Nota Privada:</span>
