...
- Observações: ${observationsText}`;
  ...
  app.get("/api/v1/organizations/:orgId/limits", async (req, res) => {
    try {
      ...
      // Send the response
      // Global roles authorize operational traversal, but product entitlement is
      // a tenant benefit and therefore comes exclusively from the tenant owner.
      const ecosystemAccess = await organizationHasEcosystemAccess(db, orgId, orgSnap.data());
