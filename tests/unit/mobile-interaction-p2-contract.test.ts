...
describe('mobile interaction P2 contract', () => {
  it('keeps release presentation off the public bootstrap and removes first-access welcome state', () => {
    const news = read('hooks/useNews.ts');
    const app = read('App.tsx');

    expect(news).toContain('useState<string[]>(readStoredSeenNewsIds)');
    expect(news).not.toContain('readStoredWelcomeDismissed');
  ...
  it('keeps automatic feature-release state isolated from the global modal context', () => {
  ...
  it('keeps the release modal mobile surface opaque and its close transition short', () => {
    const modal = read('components/WhatsNewModal.tsx');

    expect(modal).toContain('bg-black/78');
    expect(modal).toContain('sm:bg-black/55');
    expect(modal).toContain('sm:backdrop-blur-md');
    expect(modal).toContain('transition={{ duration: 0.16');
    expect(modal).toContain("beginInteractionPaintMeasurement('release_news_close_to_paint_ms')");
