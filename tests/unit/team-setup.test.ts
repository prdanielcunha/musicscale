...
function createFactory(overrides: Partial<UserProfile> = {}): UserProfile {
  return {
    uid: 'user-id',
...
describe('evaluateTeamSetup', () => {
  ...
  it('2. somente usuário atual', () => {
    const result = evaluateTeamSetup([createFactory({ uid: 'current' })], 'current');
  ...
  it('3. usuário atual não conta como adicional', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'current' }),
      createFactory({ uid: 'other' })
    ], 'current');
  ...
  it('4. integrante com roleId e specialtyIds é configurado', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', roleId: 'admin', specialtyIds: ['guitar'] })
    ], 'current');
  ...
  it('5. integrante com musicscaleRole e specialtyIds é configurado', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', musicscaleRole: 'musician', specialtyIds: ['vocals'] })
    ], 'current');
    expect(result.memberStatuses[0].isConfigured).toBe(true);
  });
  ...
  it('6. organizationRole sozinho não configura acesso', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', organizationRole: 'admin', specialtyIds: ['vocals'] })
    ], 'current');
  ...
  it('7. systemRole sozinho não configura acesso', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', systemRole: 'admin', specialtyIds: ['vocals'] })
    ], 'current');
    expect(result.memberStatuses[0].hasAccessProfile).toBe(false);
  });

  it('8. role legado sozinho não configura acesso', () => {
    // using ts-ignore because role is a legacy property that might not be in UserProfile typing but mentioned in prompt
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', role: 'admin', specialtyIds: ['vocals'] })
    ], 'current');
    expect(result.memberStatuses[0].hasAccessProfile).toBe(false);
  });
  ...
  it('9. roleId vazio não configura acesso', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', roleId: '', specialtyIds: ['vocals'] })
    ], 'current');
    expect(result.memberStatuses[0].hasAccessProfile).toBe(false);
  });
  ...
  it('10. musicscaleRole com espaços não configura acesso', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', musicscaleRole: '   ', specialtyIds: ['vocals'] })
    ], 'current');
    expect(result.memberStatuses[0].hasAccessProfile).toBe(false);
  });
  ...
  it('11. specialtyIds vazio não configura funções', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', roleId: 'admin', specialtyIds: [] })
    ], 'current');
  ...
  it('13. IDs ministeriais duplicados não alteram a conclusão', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', roleId: 'admin', specialtyIds: ['vocals', 'vocals'] })
    ], 'current');
  ...
  it('14. integrante somente com acesso é incompleto', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', roleId: 'admin' })
    ], 'current');
    expect(result.memberStatuses[0].isConfigured).toBe(false);
  });
  ...
  it('15. integrante somente com função é incompleto', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'other', specialtyIds: ['vocals'] })
    ], 'current');
    expect(result.memberStatuses[0].isConfigured).toBe(false);
  });
  ...
  it('17. duas pessoas configuradas são contadas corretamente', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'a', roleId: 'admin', specialtyIds: ['v'] }),
      createFactory({ uid: 'b', roleId: 'leader', specialtyIds: ['g'] })
    ], 'current');
  ...
  it('21. UID repetido não é contado duas vezes', () => {
    const result = evaluateTeamSetup([
      createFactory({ uid: 'a', roleId: 'admin' }),
      createFactory({ uid: 'a', roleId: 'leader', specialtyIds: ['g'] })
    ], 'current');
    expect(result.totalMembers).toBe(1);
    expect(result.memberStatuses[0].hasMinistryFunctions).toBe(false);
  });
