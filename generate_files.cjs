...
runTests();`
  ...
  'test_tenant_boundaries.ts': {
    target: '8d63b71d8845b0e499ff2bd86d03b808c50e5b3a',
...
for (const [filename, info] of Object.entries(files)) {
  ...
  for (let i = 0; i < variations.length; i++) {
    const h = getGitHash(variations[i]);
    if (h === info.target) {
      fs.writeFileSync(filename, variations[i]);
      console.log(`Matched ${filename} -> ${info.target} (var ${i})`);
      matched = true;
      break;
    }
  }
  if (!matched) {
    console.error(`Failed to match ${filename} (target: ${info.target})`);
    console.error(`Base hash: ${getGitHash(info.content)}`);
    console.error(`LF hash: ${getGitHash(info.content + '\n')}`);
    allMatched = false;
  }
}
