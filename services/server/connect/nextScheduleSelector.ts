...
export interface ConnectScaleRecord {
  ...
  songIds?: string[] | null;
  medleys?: { id: string; steps: { songId: string }[] }[] | null;
...
/**
 * Server-side, tenant-safe equivalent of the personal "assigned-event" rule
 * already used by the MusicScale Home experience.
 *
 * When a MusicScale points to a valid BandScale from the same organization,
 * that BandScale is the assignment source. Otherwise eventAssignments are
 * used, matching the current Home fallback semantics.
 */
export function selectNextAssignedSchedule(
  ...
  const bandScalesById = new Map(
    input.bandScales
      .filter(
        (bandScale) =>
          bandScale.id && bandScale.organizationId === organizationId,
      )
  ...
  for (const scale of candidates) {
    ...
    if (linkedBandScale) {
      ...
      if (actorAssignments.length > 0) {
        return {
          ...
          functionNames: uniqueStrings(
            actorAssignments.map((assignment) => assignment.instrumentId),
          ),
        };
      }
    ...
    if (actorAssignments.length > 0) {
      return {
        ...
        functionNames: uniqueStrings(
          actorAssignments.map((assignment) => assignment.functionName),
        ),
      };
    }
  }
