...
let legacy = fs.readFileSync('scripts/apply-draft-lifecycle-hotfix.mjs', 'utf8');

legacy = legacy.replace(
  /patch\('services\/server\/scale\/musicScaleCommandService\.ts', \[[\s\S]*?\n\]\);\n\n/,
  ''
);
...
console.log('Publish payload now preserves timeZone without changing notification query semantics.');
